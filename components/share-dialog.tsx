...
export function ShareDialog({ document, shares }: ShareDialogProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setEmail("")
      toast.success(`Document shared with ${email}`, { id: toastId })
      window.location.reload()
    } catch (error) {
      console.error("Share error:", error)
  ...
  const handleUnshare = async (shareId: string, userEmail: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Unshare failed")

      toast.success(`Access removed for ${userEmail}`, { id: toastId })
      window.location.reload()
    } catch (error) {
              ...
              {shares.map((share) => (
                ...
                >
                  ...
                  <Button
                    ...
                    variant="ghost"
                    className="h-8 w-8"
                    onClick={() => handleUnshare(share.id, share.user.email)}
                  >
                    <X className="h-4 w-4" />
                  </Button>
                </div>
